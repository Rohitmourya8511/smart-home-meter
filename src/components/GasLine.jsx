...
const GasLine = ({ isCustomLineColors = false, isDashboard = false }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get("https://api.thingspeak.com/channels/1701501/feeds.json?results=50"); // Replace with your API endpoint
  ...
  const formatData = (apiData) => {
    // Format the API response data to match Nivo's data structure
    return [
      {
        id: "GAS Usage",
  ...
  const dataToShow = gasData.map((dataset) => ({
    ...dataset,
  ...
  return (
    <ResponsiveLine
      ...
      axisLeft={{
        ...
        tickPadding: 5,
        tickRotation: 0,
        legend: isDashboard ? undefined : "Volume", // added
        legendOffset: -40,
        legendPosition: "middle",
      }}
      enableGridX={false}
